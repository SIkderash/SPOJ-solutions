...
#define sfor(j,n,k)         for(int i=j; i<n; i+=k)
#define rfor(j,n,k)          for(int i=n-1; i>=j; i+=k)
...
template <typename T> bool bitcheck(T p,T pos)
{
    return (bool)(p&(1<<pos));
}
template <typename T> T biton(T p,T pos)
{
    return p=p|(1<<pos);
}
template <typename T> T bitoff(T p,T pos)
{
    return p=p&~(1<<pos);
}
template <typename T> ll toint(T s)
{
    ll p;
    stringstream ss(s);
    ss>>p;
...
const int X[]= {+1,-1,0,0,-1,+1,0,0};
const int Y[]= {+0,0,+1,-1,0,0,+1,-1};
const int mx        = 1e5+10;
...
///code start here
int n, m, k, root_x, root_y, trs_x, trs_y;
char g[45][45];
int d[45][45];
int val(int x, int y){
    if(g[x][y]=='s') return 1;
...
bool bfs(int x, int y){
    ...
    for(int i=0; i<4; i++){
        ...
        if(valid(u,v)){
            if(g[u][v]!='#')
            if(d[u][v]>d[x][y]+val(u,v)){
                d[u][v] = d[x][y]+val(u,v);
                if(g[u][v]=='@' && d[u][v]*2<=k) return true;
                if(val(u,v)==0)q.push_front(make_pair(u,v));
...
int main()
{
    ...
    for(int i=0; i<n; i++)
        ...
        {
            cin >> g[i][j];
            if(g[i][j]=='@') {root_x=i; root_y=j;}
            else if(g[i][j]=='x') {trs_x = i; trs_y = j;}
            d[i][j] = INT_MAX;
        }
    if (bfs(trs_x,trs_y)) cout << "SUCCESS";
