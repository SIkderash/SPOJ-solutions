...
int n, m, vis[200][200],f[200][200];
int dis[200][200] ,wt[200][200];
char g[200][200];
void bfs (int x, int y)
{
    ...
    while( !pq.empty ())
    {
        int u = get<1>(pq.front());
        int v = get<2>(pq.front());
        pq.pop();
        //cout << u <<"  "<< v <<nl;
        for( int i=0; i<4; i++)
        {
            ...
            int z = v + Y[i];
            //cout << w << " " <<z << nl;
                     if( valid(w,z) && !vis[w][z] && dis[ w ][ z ]>dis[u][v] + wt[w][z] )
                    {
                        dis[ w ][ z ] = wt[w][z] + dis[u][v] ;
                        vis[w ][z]=1;
              //        cout << dis[ w ][ z ]<<nl;
                        pq.push(make_tuple(wt[w][z], w, z));
