...
pll p = fib(n>>1);
ll c = (p.first*((((p.second*2LL)%mod-p.first)+mod)%mod))%mod;
ll d = ((p.second*p.second)%mod + (p.first*p.first)%mod)%mod;
if(n&1) return {d,(c+d)%mod};
    ...
    int main()
    {
        ...
        ll n, m;
        cin >> n >> m;
        ll sum_n = fib(n+1).first-1;
        ll sum_m = fib(m+2).first-1;
        cout <<((sum_m-sum_n)%mod+mod)%mod<<nl;
        }
